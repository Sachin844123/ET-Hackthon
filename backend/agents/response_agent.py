...
class AutonomousResponseAgent:
    ...
    async def evaluate_and_respond(
        ...
        if not requires_approval or blast_radius == "LOW":
            # Execute immediately
            await self._execute_playbook(execution, alert, playbook)
        else:
            # Queue for human approval
            execution.status = "PENDING_APPROVAL"
            await self._store_pending(execution)
            logger.info(
                "Playbook '%s' queued for approval (blast_radius=%s, execution_id=%s)",
                playbook_name,
                blast_radius,
    ...
    @staticmethod
    def _build_context(alert: RetroactiveAlert) -> dict[str, str]:
        """Extract substitution variables from an alert."""
        pf = {}  # parsed_fields not available on RetroactiveAlert
        return {
            "source_entity": "AIIMS-PATIENT-MGMT-01",  # demo default
